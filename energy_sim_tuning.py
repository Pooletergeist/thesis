...
def write_pop_counts_to_file(FILENAME,  
                            NODE_LIST, 
                            CLONE_ID_LIST,
                            R_AMT, H_AMT,
                            GENS, SEED,
    ...
    for clone_ix in range(len(NODE_LIST)):
        ...
        # make table line
        line = (comma_join([clone_name, divRt, movRt, live_pop, historic_pop]) +                 shared_suffix)
        # write line
        append_str_to_file(FILENAME, line, DIRECTORY)
    # write totals
    totals_line = (comma_join(["all", -1, -1, 
...
def run_and_record_simulation(W, H, RSRC_AMT, RSRC_IV, HZRD_AMT, 
    ...
    # build sim model
    sim_state = energy_model(W=W, H=H, RSRC_AMT = RSRC_AMT, 
...
def seeded_grid_search(SEED, N_SEEDS, # only these used here, rest are passed
    ...
    COLNAMES = ["clone", "divRt", "movRt", "livePop", "historicPop",
